...
public class Search_EditText extends android.support.v7.widget.AppCompatEditText {
    ...
    public Search_EditText(Context context) {
        super(context);
        init();
        //初始化该组件时，对Search_EditText进行初始化 -->步骤2
    }
    ...
    private void init() {
        clearDrawable = getResources().getDrawable(R.drawable.ic_edit_delete);
        //searchDrawable = getResources().getDrawable(R.drawable.ic_back);
    ...
    protected void onFocusChanged(boolean focused, int direction, Rect previouslyFocusedRect) {
        ...
        // focused = 是否获得焦点
        //同样根据 setClearIconVisible()判断是否要显示图标
    }
    ...
    /**
     * 步骤4.对删除图标区域设置点击事件，即 " 点击 = 清空搜索框 "
     * 原理：当手指抬起的位置再删除图标区域，即视为点击了删除图标 = 清空搜索框
     * */
